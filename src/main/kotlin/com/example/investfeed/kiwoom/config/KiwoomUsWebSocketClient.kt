...
@Component
class KiwoomUsWebSocketClient: WebSocketClient(URI("wss://api.kiwoom.com:10000/api/us/websocket")) {
        ...
        try {
            ...
            if(rootNode.has("return_code")) {
                if(rootNode.get("return_code")?.asInt() != 0) {
                    ...
                    log.error { "US WebSocket onMessage : $return_msg" }
    ...
    override fun onClose(code: Int, reason: String?, remote: Boolean) {
        log.info { "US WebSocket onClose : $reason" }
    }

    override fun onError(e: Exception?) {
        log.error { "US WebSocket onError : $e" }
    }
